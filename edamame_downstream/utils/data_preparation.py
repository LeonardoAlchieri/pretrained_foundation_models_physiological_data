...
from edamame_downstream.preprocessing.decomposition import decomposition
from edamame_downstream.preprocessing.filter import butter_lowpass_filter_filtfilt
...
def split_data_into_sessions(data: np.ndarray, session_data: pd.DataFrame) -> dict:
    ...
    for _, row in session_data.iterrows():
        ...
        selected_data = data[(data["Timestamp"] >= start_time) & (data["Timestamp"] <= end_time)]
...
def empty_scaler(data: np.ndarray, *args, **kwargs) -> np.ndarray:
    return data
