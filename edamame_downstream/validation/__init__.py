...
class TACV:
    ...
    def __call__(self, data: DataInfo) -> tuple[list[DataInfo], list[DataInfo]]:
        ...
        labels = data["labels"]
        groups = data["groups"]
        gkf = GroupKFold(n_splits=self.num_folds, random_state=self.random_state, shuffle=True)
...
class LOPO:
    ...
    def __call__(self, data: DataInfo) -> tuple[list[DataInfo], list[DataInfo]]:
        ...
        train_folds = []
        test_folds = []
        for group in tqdm(unique_groups, desc="LOPO Splitting", total=len(unique_groups)):
